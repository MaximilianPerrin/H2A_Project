...
HistoneTreePath <- "/Users/perrinm/Documents/AHocher_Project/TREE/H2A_TREE/Individual_Euk_HF_hits_PostProcessed_H2A_HMM_Subset_Curated_wHmfAB_mafftaligned.fa.clipkit.Rooted.treefile.txt"
  ...
  #Group by species, keep only species that contain at least one sq, keep only the sequences that do not contain sq (in species that do)
  H2AX_to_prune <- H2AX_to_prune %>%
    group_by(Species) %>%
    filter(any(contains_sq == TRUE)) %>%
    filter(contains_sq == FALSE)
  ...
  write.tree(PrunedTree,paste0("/Users/perrinm/Documents/AHocher_Project/TREE/",HistoneType,"_H2AX_Forced_3.tree"))
  ...
  #Correct FASTA names
  TipLabel <- data.frame(PrunedTree$tip.label)
  View(TipLabel)
  H2A_names <-data.frame(getName(H2AFastaSeq))
  H2A_names$Species <- gsub(".*\\$(.*?)@.*","\\1", H2A_names$getName.H2AFastaSeq.) 
  TipLabel$species=gsub(".*\\$(.*?)@.*","\\1", TipLabel$PrunedTree.tip.label)
  TipLabel <- TipLabel$species[!TipLabel$species %in% H2A_names$Species]
  TipLabel <- data.frame(TipLabel)
  ...
  write.fasta(sequences = H2AFastaSeq, 
              ...
              file.out = "H2A_H2AX_Forced_3.fasta")
